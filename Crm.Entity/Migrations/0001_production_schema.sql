/*
    ...
    ОБНОВЛЕНИЕ: Блок 2 (CompanyId в Clients, новые поля Deal) и Блок 3 (новые
    таблицы Supplier/Component/ProductTemplate/.../ContractorAccessToken) ТЕПЕРЬ
    ПРИМЕНЯЮТСЯ АВТОМАТИЧЕСКИ при старте приложения — см. Crm/Program.cs, блок
    "Производственный модуль" (идемпотентные IF OBJECT_ID/COL_LENGTH проверки,
    как уже было заведено в проекте для таблиц Deal/ChatMessage). Просто
    запустите приложение с connection string, указывающим на OMSIT — схема
    досоздастся сама. Эти блоки оставлены здесь же для справки/ручного прогона
    на другом окружении, где приложение ещё не запускали.

    ВРУЧНУЮ НУЖНО ПРИМЕНИТЬ ТОЛЬКО БЛОК 1 (он деструктивный, поэтому НЕ включён
    в автозапуск приложения):
    ...
    - Применяется через SSMS/sqlcmd к базе `crm` на сервере OMSIT (проект
      database-first, это не EF Core Migration).

    Также: после того как приложение досоздаст CompanyId в Clients, существующие
    строки получат CompanyId = NULL — их нужно вручную разнести по компаниям
    (или проставить компанию по умолчанию), прежде чем делать колонку NOT NULL
    (это не делается автоматически, чтобы не потерять данные).
*/
